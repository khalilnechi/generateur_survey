...
class NavigationBar extends React.Component {
  ...
  componentWillMount() {
    firebase.database().ref("Admin_mails").on('value',(mails)=>{
      if(mails.val().includes(this.props.user.email))
       this.setState({admin:true})
      else
      this.setState({admin:false})
    })
    firebase.database().ref('Table_Users').on('value', (Users_registered) => {
      //console.log("Users_registered=" + JSON.stringify(Users_registered))
      //console.log("Users_registered="+Users_registered)
      this.state.Users_registered = [Users_registered.val()]
      Users_registered.forEach((User_registered) => {
        //console.log("User_registered" + JSON.stringify(Users_registered))
  ...
  render() {
    //console.log("this.state.Users_registered=" + JSON.stringify(this.state.Users_registered))

    /*  <Nav.Link href="/Analyse"
            style={{ color: this.props.Activelink === "Analyser les résultats" ? "white" : null }}>Analyser les résultats</Nav.Link>*/
    return (
...
{
  this.props.user.email==="nechikhalili@ensit.u-tunis.tn"?
<Nav.Link onClick={
            () => {
        ...
        >
          ...
          <Modal.Body style={{ paddingLeft: 100 }}>
            ...
            {
              this.state.Users_registered.map((el) => {
               // console.log(el.key)
                if(el.key)
                return (
                  <Fade in={true}>
