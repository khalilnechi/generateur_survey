...
class Consultation_survey extends React.Component {
    ...
    setTableSurvey(sondages) {
        sondages.forEach(child => {
            ...
            var privateAccess
            if(child.val().Statut_sondage==="Privé")
              {
            console.log("mail=" + this.state.user.email + ' ' + child.val().private_viewers.includes("hajer.rihani@newaccess.ch"))

                privateAccess= child.val().private_viewers.includes("hajer.rihani@newaccess.ch")
              }
              else
              privateAccess=true
