        ...
        try {
            ...
            firebase.database().ref("Sondage/" + id).once('value', (Sondage) => {
                ...
                this.state.table_questions.forEach((q, indexQuestion) => {
                    ...
                    this.state.Tab_Analyse[indexQuestion] = []
                    q.radioTab.forEach((radio) => {
                ...
                this.state.Tab_reponses.forEach((rep)=>{
                    rep.UserResponse.forEach((question,indexQuestion)=>{
                        if(question.type_rep==="Choix multiple")
                         this.state.Tab_Analyse[indexQuestion][question.value].y++
                        else
                        this.state.Tab_Analyse[indexQuestion]=0
                        //else
        ...
        return (
            ...
            <div className="App">
                ...
                <div
                    ...
                    <Row>
                        <Col>
                            <text style={{ fontSize: 70, fontWeight: "bold", color: "gray" }}>{this.state.nombre_reponses}</text><p>Réponses</p>
                        </Col>
                        <Col>
                            Titre :
                    <text style={{ fontSize: 20, fontWeight: "bold" }}> {this.state.Titre}</text>
                            <p>
                                Type :
                    <text style={{ fontSize: 20, fontWeight: "bold" }}>  {this.state.Type}</text></p>
                            <p>
                                Crée le :
                    <text style={{ fontSize: 20, }}>  {this.state.DateSondage + " à " + this.state.TimeSondage}</text></p>
                            <p>
                                Statut :
                    <text style={{ fontSize: 20, }}>  {this.state.Statut_sondage}</text></p>
                    ...
                    <div className="d-flex flex-column">
                        <ButtonGroup size="lg">
                            <Button
                            disabled={this.state.btnAnalyser}
                            onClick={() => {
                                //console.log("dsfqfqfqfqsfdsfffffffffffffffffffff")
                                this.setState({
                                    btnAnalyser: true
                                })
                            }}>
                ...
                </div>
                {
                    this.state.btnAnalyser ?
                        <div>
                            {
                                this.state.Tab_Analyse.map((Question, index) => {
