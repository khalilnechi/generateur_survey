...
class Consultation_survey extends React.Component {
    ...
    importerTablesurvey(all) {
        ...
        firebase.database().ref('Sondage').orderByChild("Statut_sondage").equalTo("Privé").once('value', (sondagesPrivate) => {
            var TousSondages = sondagesPrivate
            console.log("sondagesPrivate:" + JSON.stringify(sondagesPrivate))
            /*var a = sondagesPrivate.val()
            console.log("jparse:" + JSON.stringify(a["66"]))*/

            sondagesPrivate.forEach((sp) => {
                ...
                if (sp.val().private_viewers.includes(this.state.user.email))
                    ...
                    console.log("no")
                TousSondages=JSON.parse("66:sp.val()")

            })
            console.log("TousSondages:" + JSON.stringify(TousSondages))
