...
class Consultation_survey extends React.Component {
    ...
    importerTablesurvey(all) {
        ...
        //importation des sondages public
        ...
        firebase.database().ref('Sondage').orderByChild("Statut_sondage").equalTo("Privé").once('value', (TousSondagesPrivate) => {
            var sondagesPrivate= TousSondagesPrivate
            console.log("sondagesPrivate:" + JSON.stringify(sondagesPrivate))
            TousSondagesPrivate.forEach((sp) => {
                console.log("sondage " + " : " + JSON.stringify(sp.val().private_viewers))
                console.log("this.state.user=" + this.state.user.email)
               /* if (sp.val().private_viewers.includes(this.state.user.email))
                    ...
                    TousSondagesPrivate.ages.filter(checkAdult);
                    

            })
            var j=sondagesPrivate
            console.log("j="+ j.val(0))
            this.setTableSurvey(TousSondages)
        });
    }
    componentWillMount() {
        console.log("componentDidMount:consultation_survey")
        ...
        if (this.state.user === null)
            ...
            return (
                <div className="App">
                    ...
                    <text style={{ color: this.state.TousSondages ? "black" : "gray" }}>Tous les sondages</text>
                    ...
                    <text style={{ color: this.state.TousSondages ? "gray" : "black" }}>Seulement mes sondages</text>
                        ...
                        <Row style={{ textAlign: "center" }}>
                            {
                                ...
                                this.state.table_survey !== undefined ?
                                    ...
                                    this.state.table_survey.map((Item, key) => {
                                        ...
                                        // console.log('Item.owner_uid='+Item.owner_uid)
                                                    ...
                                                    <CardActions>
                                                        {
                                                            Item.hasAlreadyresponded ?
                                                                <text style={{ color: "#487eb0" }}>Vous avez déja répondu</text>
                                                                :
